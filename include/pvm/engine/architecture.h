...
namespace ngu::pvm {
    namespace detail {
        ...
        consteval void shuffle_indices(
            std::uint8_t* arr,
    ...
    /**
     * @brief Describes the ISA (Instruction Set Architecture) and ABI (Application Binary Interface).
     *
     * The entire architecture is evaluated at compile time (consteval).
     *
     * @par Instruction Set (ISA)
     * Instruction format:       @c OPCODE(0) | INSN_SIZE(8) | MODE(11) | DESTINATION(12) | SOURCE(16) @n
     * Without immediate:        @c BYTE(2B), @c WORD(2B), @c DWORD(4B) @n
     * With immediate:           @c DWORD_B(5/1B), @c DWORD_W(6/2B), @c DWORD_D(8/4B), @c DWORD_Q(12/8B) @n
     * Addressing modes:         @c REGISTER, @c IMMEDIATE @n
     * Registers:                @c R0–R11 - 12 general-purpose registers; @c FLAGS and @c PC - special-purpose. @n
     * Meta-opcodes:             @c OP_LABEL, @c OP_JMPL, @c OP_JEL, @c OP_JNEL - markers resolved into real
     *                           branch instructions on the final pass. @n
     * Standard opcodes:         21 opcodes describing the VM execution logic (@c MOV, @c ADD, @c SUB, ...).
     *
    ...
    public:
        template<bool HasImmediate = false> struct instruction {
            ...
            enum class insn_size : std::uint8_t {
                BYTE, WORD, DWORD, DWORD_B, DWORD_W, DWORD_D, DWORD_Q,
            };
        ...
        struct registers {
            enum logical_reg : std::uint8_t {
                REG_R0, REG_R1, REG_R2, REG_R3, REG_R4, REG_R5, REG_R6, REG_R7, REG_R8, REG_R9, REG_R10, REG_R11,
                REG_FLAGS, REG_PC,
                REG_MAX
            };
        ...
        struct opcodes {
            ...
            enum logical_op : std::uint8_t {
                ...
                OP_SHL, OP_SHR, OP_ROL, OP_ROR,
                OP_CMP,
                OP_JMP, OP_JMPA, OP_JMPI,
                OP_JNE, OP_JE, OP_JNEI, OP_JEI,
                OP_NOP, OP_HALT,
                OP_MAX,
            };

            static_assert(static_cast<std::uint8_t>(OP_MAX) < static_cast<std::uint8_t>(OP_META_START), "Opcodes out of bounds");
         ...
         * Registers and opcodes are shuffled independently: opcodes use @c lcg_next(entropy) as seed.
         *
...
#define INIT_REG(path, idx) arch.regs.path = reg_perm[idx]
        ...
        INIT_REG(gpr.R0, reg::REG_R0);   INIT_REG(gpr.R1, reg::REG_R1);   INIT_REG(gpr.R2, reg::REG_R2);
        INIT_REG(gpr.R3, reg::REG_R3);   INIT_REG(gpr.R4, reg::REG_R4);   INIT_REG(gpr.R5, reg::REG_R5);
        INIT_REG(gpr.R6, reg::REG_R6);   INIT_REG(gpr.R7, reg::REG_R7);   INIT_REG(gpr.R8, reg::REG_R8);
        INIT_REG(gpr.R9, reg::REG_R9);   INIT_REG(gpr.R10, reg::REG_R10); INIT_REG(gpr.R11, reg::REG_R11);
...
#undef INIT_REG
        ...
        for (std::size_t i{}; i < reg::REG_MAX; ++i) {
            arch.regs.physical_map[i] = mapping_entry(
                static_cast<reg>(i),
                reg_perm[i]
            );
        }
...
#define INIT_OP(name, idx) arch.ops.name = op_perm[idx]
        ...
        INIT_OP(ADD, op::OP_ADD);   INIT_OP(SUB, op::OP_SUB);
        INIT_OP(AND, op::OP_AND);   INIT_OP(OR, op::OP_OR);     INIT_OP(XOR, op::OP_XOR);   INIT_OP(NOT, op::OP_NOT);
        INIT_OP(SHL, op::OP_SHL);   INIT_OP(SHR, op::OP_SHR);   INIT_OP(ROL, op::OP_ROL);   INIT_OP(ROR, op::OP_ROR);
        INIT_OP(CMP, op::OP_CMP);
        INIT_OP(JMP, op::OP_JMP);   INIT_OP(JMPA, op::OP_JMPA); INIT_OP(JMPI, op::OP_JMPI);
        INIT_OP(JNE, op::OP_JNE);   INIT_OP(JE, op::OP_JE);     INIT_OP(JNEI, op::OP_JNEI); INIT_OP(JEI, op::OP_JEI);
        INIT_OP(NOP, op::OP_NOP);   INIT_OP(HALT, op::OP_HALT);

#undef INIT_OP

        for (std::size_t i{}; i < op::OP_MAX; ++i) {
            arch.ops.physical_map[i] = mapping_entry(
                static_cast<op>(i),
                op_perm[i]
            );
        }
    ...
    namespace detail {
        ...
        PVM_FORCEINLINE constexpr std::uint8_t resolve_meta(const arch::opcodes& ops, arch::meta_op m) {
            for (const auto& [from, to] : arch::opcodes::meta_map)
                if (from == m) return ops.physical_map[to].to;
            return 0;
        }

        PVM_FORCEINLINE constexpr arch::insn_size calc_insn_size(const std::uint64_t val, const bool is_signed = false) {
            if (is_signed) {
