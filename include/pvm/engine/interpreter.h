...
namespace ngu::pvm {
    ...
    *
    * @par Fields
    * @c arch_ - copy of the @ref architecture object; used to resolve physical opcodes in @ref dispatch(). @n
    * @c ctx_  - execution state; marked @c mutable since @ref run() is const but modifies the context.
    *
    * @par Execution
   ...
   public:
      ...
      explicit consteval interpreter(const architecture &arch) :
         arch_(arch), ctx_(arch_) {}
      ...
      /// @brief Returns a pointer to the mutable execution context. @return Pointer to @ref context.
      context* get_ctx() const {
         return &ctx_;
      }

   private:
      PVM_FORCEINLINE status dispatch(
         context* ctx,
         const insn_view& insn,
         const insn_stream_rt& insn_entries
         ) const;
   ...
   inline interpreter::status interpreter::dispatch(
      ...
      // but since arch_ is a class field, it is accessed through a runtime object (this), which by definition cannot be a constant expression.
      // Building a { opcode - dispatcher } table would likely introduce overhead for 21 opcodes and negatively affect performance.
      // The decision regarding a table-based approach may be revisited in the future after further benchmarking.
